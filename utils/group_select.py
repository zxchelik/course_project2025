...
class GroupSelector:
    ...
    users: dict[int, UserIdFioModel] = {}
    help_users: dict[int, UserIdFioModel] = {}
    ...
    def get_result(self) -> list[UserIdFioModel] | tuple[list[UserIdFioModel], list[UserIdFioModel]]:
        if self.is_help_enabled:
            return [self.user] + list(self.users.values()), list(self.help_users.values())
        else:
            return [self.user] + list(self.users.values())
    ...
    async def _process_select(self, callback: CallbackQuery, callback_data: GroupSelectorCallbackData) -> None:
        ...
        if isinstance(value, int):
            ...
            if user.id in self.users:
                ...
                if self.is_help_enabled:
                    self.help_users[user.id] = user
            elif user.id in self.help_users:
                self.help_users.pop(user.id)
            else:
    ...
    def _get_text(self) -> str:
        ...
        res += sep + sep.join([i.fio for i in self.users.values()])
        if self.help_users:
            res += "\n" + ("(П)" + sep).join([i.fio for i in self.help_users.values()]) + "(П)"
        return res
