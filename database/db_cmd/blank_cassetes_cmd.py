...
class TaskModel(BaseModel):
    ...
    def to_list_for_excel(self):
        return [self.id, self.cassette_name, self.type, self.quantity, None, self.priority, None,
...
async def add_tasks(session: AsyncSession, tasks: List[AddTaskModel], customer_id: int) -> None:
    blank_cassettes = []
    for task in tasks:
        blank_cassettes.append(
            BlankCassettes(**task.dict(), customer_id=customer_id)
        )
...
@async_session_context
async def edit_tasks(session: AsyncSession, task_updates: List[EditTaskModel],
                     task_deletes: List[DeleteTaskModel] | None) -> None:
    update_mappings = [i.to_dict() for i in task_updates]
    await session.execute(
        update(BlankCassettes), update_mappings
    )
    if task_deletes:
        await session.execute(
            delete(BlankCassettes).where(BlankCassettes.id.in_([i.id for i in task_deletes]))
        )
