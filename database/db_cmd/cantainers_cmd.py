...
async def add_container(session: AsyncSession, number, date_cont, name, color, weight, batch_number, cover_article,
                        comments, users_id, storage="База"):
    group, percent = await create_group(users_id=users_id)

    container = Container(number=number, date_cont=date_cont, name=name, color=color, weight=weight,
                          batch_number=batch_number, cover_article=cover_article, comments=comments, percent=percent,
                          storage=storage)
...
async def get_all_container_by_month(session: AsyncSession, year, month) -> dict:
    query = (select(Container)
             .where(extract("MONTH", Container.date_cont) == month,
                    extract("YEAR", Container.date_cont) == year).options(selectinload(Container.group))
