...
text2 = (
    lambda quantity: f"""Выберите количество:
{quantity} шт."""
)
text3 = lambda date: f"Дата: {date.strftime('%d.%m.%Y')}"
text4 = (
    lambda date: f"""Выберите дату:
{date.strftime("%d.%m.%Y")}"""
)
...
text6: Callable[[TaskModel, int, date_type, str], str] = lambda task, quantity, date, user_fio: (
    "🛠️Нарезаны детали:\n" + full_info(task, quantity, date) + f"\nОтветственный: {user_fio}"
)
full_info: Callable[[TaskModel, int, date_type], str] = (
    lambda task, quantity, date: f"""<pre>Наимен:{task.cassette_name}
Кол-во:{quantity}/{task.quantity}
Дата  :{date}
Тех комментарий:{task.technical_comment}
                ...
                inline_keyboard=[
                    [
                        ...
                        InlineKeyboardButton(
                            text="Подтвердить✅", callback_data=SelectBlankCassette(action="confirm").pack()
                        ),
                    ]
                ]
            )
