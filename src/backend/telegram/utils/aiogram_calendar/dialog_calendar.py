...
class DialogCalendar:
    ...
    async def start_calendar(self, year: int = datetime.now().year) -> InlineKeyboardMarkup:
        ...
        for value in range(year, year + 5):
            inline_row.append(InlineKeyboardButton(text=value, callback_data=build_data("SET-YEAR", value, -1, -1)))
        # nav buttons
        ...
        inline_kb.append(inline_row)
        inline_row = []
        inline_row.append(InlineKeyboardButton(text="<<", callback_data=build_data("PREV-YEARS", year, -1, -1)))
        inline_row.append(InlineKeyboardButton(text=">>", callback_data=build_data("NEXT-YEARS", year, -1, -1)))
        inline_kb.append(inline_row)
    ...
    async def _get_month_kb(self, year: int):
        ...
        inline_row.append(InlineKeyboardButton(text=year, callback_data=build_data("START", year, -1, -1)))
        inline_row.append(InlineKeyboardButton(text=" ", callback_data=ignore_callback))
        ...
        for month in self.months[0:6]:
            inline_row.append(
                InlineKeyboardButton(
                    text=month, callback_data=build_data("SET-MONTH", year, self.months.index(month) + 1, -1)
                )
            )
        inline_kb.append(inline_row)
        inline_row = []
    ...
    async def _get_days_kb(self, year: int, month: int):
        ...
        inline_row.append(InlineKeyboardButton(text=year, callback_data=build_data("START", year, -1, -1)))
        inline_row.append(
            InlineKeyboardButton(text=self.months[month - 1], callback_data=build_data("SET-YEAR", year, -1, -1))
        )
        inline_kb.append(inline_row)
        inline_row = []
        ...
        for week in month_calendar:
            ...
            for day in week:
                if day == 0:
                    ...
                    continue
                inline_row.append(
                    InlineKeyboardButton(text=str(day), callback_data=build_data("SET-DAY", year, month, day))
                )
            inline_kb.append(inline_row)
