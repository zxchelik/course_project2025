...
async def select_name_pattern(parent_id: Union[int, None] = -1, page: int = 0, page_size: int = 5,
                              base: Union[int, None] = None, can_edit: bool = False):
    if parent_id == -1:
        parent_id = base
    ...
    if _names:
        ...
        if page == total_pages and can_edit:
            builder.row(InlineKeyboardButton(text="✏️Добавить", callback_data=f'add_{parent_id}'),
        ...
        if parent_id != base:
            row.append(InlineKeyboardButton(text="⏪Назад",
                                            callback_data=SelectName(page=0,
                                                                     parent_id=await get_parent_id(parent_id)).pack()))
...
async def select_many_names_pattern(base: int | None, parent_id: int | None, selected_list: list[int] | None = None,
                                    page: int = 0,
    ...
    if names:
        for name in names:
            ...
            if name.id in selected_list:
                prefix = '✅'
            builder.row(
                InlineKeyboardButton(text=prefix + name.name,
                                     callback_data=SelectName(page=0, parent_id=name.id).pack()))
...
async def select_name_additional_fub(selected_list: list[int] | None = None, parent_id: Union[int, None] = -1,
                                     page: int = 0,
                                     page_size: int = 5):
    return await select_many_names_pattern(base=3, parent_id=parent_id, page=page, page_size=page_size,
                                           selected_list=selected_list)


async def select_name_hourly_fub(selected_list: Union[list[int], None] = None, parent_id: Union[int, None] = -1,
                                 page: int = 0,
                                 page_size: int = 5):
    return await select_many_names_pattern(base=4, parent_id=parent_id, page=page, page_size=page_size,
                                           selected_list=selected_list)
...
async def select_group_fub(user_id: int, page: int = 0, page_size: int = 5, ):
    ...
    builder.button(text="✅Закончить",
                   callback_data=SelectGroup(action='confirm'))
