...
from keyboards.inline import SelectNumber
...
async def get_select_blank_cassette_kb(page: int, page_size: int = 5) -> InlineKeyboardMarkup:
    ...
    if page > 0:
        builder.button(
            text="⬅️",
            callback_data=SelectCassette(action='ch_page', value=-1)
        )
        counter += 1
    if page < max_page:
        builder.button(
...
async def get_select_welding_addition_kb(page: int, page_size: int = 5) -> InlineKeyboardMarkup:
    ...
    for addition in additions:
        builder.button(
            text=addition[0],
            callback_data=SelectAdditional(
                action="select",
                value=addition[1]
            )
        )
    ...
    if page > 0:
        builder.button(
            text="⬅️",
            callback_data=SelectAdditional(action='ch_page', value=-1)
        )
        counter += 1
    if page < max_page:
        builder.button(
        ...
        counter += 1
    builder.button(
        text="Не выбирать дополнение",
        callback_data='gotoaddition_skip'
    )
...
def get_number_select_kb() -> InlineKeyboardMarkup:
    ...
    builder.button(
        text="-3", callback_data=SelectNumber(action="update_num", value=-3)
    )
    builder.button(
