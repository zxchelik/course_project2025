...
from database.modelsDTO.cassette import CassetteNumberModel, WeldCassetteTaskModel, RawCassetteModel, CassetteModel
from database.modelsDTO.user import UserIdFioModel
from keyboards.cassete.cutting import get_select_quantity_kb, SelectQuantity
from keyboards.cassete.welding import (
    SelectCassette,
...
from keyboards.inline import get_inline_kb, get_confirm_ikb, SelectNumber, get_confirm_date_ikb
from states.users import CassetteWelding
...
async def process_select_cassette(callback: CallbackQuery, callback_data: SelectCassette, state: FSMContext):
    ...
    match action:
        ...
        case "select":
            ...
            await callback.message.edit_text(text=f"<pre>{raw_task.to_str_table_view_welding()}</pre>", reply_markup=kb)
            ...
            for number in wct.numbers:
                if await validate_number(number=number):
                    try:
                        ...
                        text = f"<pre>{cassette_model.to_str_table_view_welding()}</pre>"
                        kb = InlineKeyboardMarkup(
                            inline_keyboard=[
                                [
                                    InlineKeyboardButton(
