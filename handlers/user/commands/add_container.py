...
import misc
from aiogram_calendar import calendar_callback_filter, SimpleCalendar
...
from keyboards.inline import get_inline_kb, get_confirm_ikb, get_select_number_fab, SelectNumber
from keyboards.name_ikb import select_name_container_fub, SelectName, SelectGroup, select_group_fub
from keyboards.reply import get_reply_keyboard
from states.users import AddContainer
...
async def start_add_container(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(text=select_date,
                                     reply_markup=await SimpleCalendar().start_calendar(year=now_date.year,
...
async def process_simple_calendar(callback_query: CallbackQuery, state: FSMContext):
    ...
    if selected:
        ...
        if (datetime.date.today() - date_).days < 2:
            ...
            await callback_query.message.answer(
                invalid_date,
                reply_markup=get_inline_kb([[("Выбрать другую дату", 'cancel')]], prefix="confirm")
            )
...
async def confirm_date(callback: CallbackQuery, state: FSMContext):
    ...
    if update_code == "cancel":
        ...
        container_numb = [year_char, 1, date.month,
                          await get_last_cont_numb(year_char=year_char, furnace=1, month=date.month), ""]
        await state.update_data(number=container_numb)

        text = select_numb.format(
            *container_numb[0:4], (container_numb[4] or " "))
        await callback.message.edit_text(text=text,
                                         reply_markup=get_select_number_fab())
...
async def get_number(callback: CallbackQuery, callback_data: SelectNumber, state: FSMContext):
    ...
    if callback_data.action == 'update_group':
        ...
        container_numb[3] = await get_last_cont_numb(year_char=container_numb[0], furnace=container_numb[1],
                                                     month=container_numb[2])
...
async def get_weigh(message: Message, state: FSMContext):
    try:
        ...
        logger.critical(e)
        await message.answer("Что-то пошло не так. Введи значение еще раз.\n"
...
async def select_page_group(callback: CallbackQuery, callback_data: SelectGroup, state: FSMContext):
    ...
    await callback.message.edit_text(text,
                                     reply_markup=await select_group_fub(page=page, user_id=callback.from_user.id))
...
{'-' * 10}+{'-' * 16}'''
    ...
    kb = get_inline_kb(rows=[
        [("❌Начать заново", "cancel"), ("Всё верно ✅", "confirm")]
    ],
        prefix='update')
...
async def get_confirm(callback: CallbackQuery, state: FSMContext):
    ...
    if update_code == 'cancel':
        ...
        data = await state.get_data()
        await add_container(date_cont=data['date'], number=data['number'], name=data['name'], color=data['color'],
                            weight=data['weigh'],
                            batch_number=data['batch_number'],
                            cover_article=data['cover_article'], comments=data['comments'],
                            users_id=data["group"])
