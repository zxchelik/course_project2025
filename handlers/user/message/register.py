...
import misc
from aiogram_calendar import dialog_callback_filter, DialogCalendar
...
from states.users import RegisterStates
...
"""
    for admin in admins:
        id = admin.tg_id
        await bot.send_message(chat_id=id, text=text, reply_markup=InlineKeyboardMarkup(one_time_keyboard=True,
                                                                                        inline_keyboard=[[
                                                                                            InlineKeyboardButton(
                                                                                                text="Показать",
                                                                                                callback_data="show_new_user")]]))
...
async def get_birthday(callback: CallbackQuery, state: FSMContext):
    ...
    if callback_data == "cancel":
        ...
        user_data = await state.get_data()
        await add_user(tg_id=user_data['id'],
                       fio=user_data["fio"],
