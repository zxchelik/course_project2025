...
from keyboards.inline import get_confirm_ikb
from states.admin import GetStats
from text_templates import confirm_date_text
from utils.containers_statistic import get_containers_statistic
...
async def get_first_date(callback: CallbackQuery, state: FSMContext):
    ...
    now_date = date.today()
    await callback.message.edit_text(text="Выберите с какого числа вывести данные",
...
async def get_last_date(callback: CallbackQuery, state: FSMContext):
    now_date = date.today()
    await callback.message.edit_text(text="Выберите по какое число вывести данные",
                                     reply_markup=await SimpleCalendar().start_calendar(year=now_date.year,
                                                                                        month=now_date.month))
    await state.set_state(GetStats.get_last_date)
