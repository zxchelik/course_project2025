...
from database.db_cmd.cantainers_cmd import move_container
...
from states.admin import GetAvailability
from utils.RAL import load_ral
...
async def send_pattern(callback: CallbackQuery, state: FSMContext):
    ...
    async def get_statistic(session: AsyncSession):
        stmt = (select(Container.name, func.count().label('count'))
    ...
    for i, c in enumerate(containers):
        ws.append(
            [c.name, c.number, c.color, c.storage, None, None, c.comments]
        )
    ...
    msg = await msg.answer_document(
        FSInputFile(name, filename=filename_),
        caption="заполните файл и пришлите его\nИли нажми на /menu"
    )
    os.remove(name)
...
Повторите попытку""" if without_rfm else "Всё готово✅"
