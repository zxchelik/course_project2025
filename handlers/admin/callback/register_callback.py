...
#     update_group
from database.db_cmd.user_cmd import get_new_user, activate_user, count_check_users, del_user as del_, update_group
from keyboards.inline import get_confirm_user_kb
from keyboards.reply import get_group_list
...
async def activate(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer("Напишите группу для данного пользователя или выберите из предложенных",
                                  reply_markup=await get_group_list())
...
async def add_group(message: Message, state: FSMContext):
    ...
    await state.clear()
    await message.answer(f"Пользователю успешно присвоена группа: {message.text}", reply_markup=InlineKeyboardMarkup(
        inline_keyboard=[
            [InlineKeyboardButton(text="Показать следующего", callback_data="show_new_user")]]))
...
async def del_user(message: Message, state: FSMContext):
    ...
    await state.clear()
    await message.answer("Мы ему сообщим его ошибку",
