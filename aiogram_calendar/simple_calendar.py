...
def process_data(data: str) -> dict: data = data.split("&")[1:]; return {"act": data[0], "year": data[1],
...
class SimpleCalendar:
    ...
    async def start_calendar(
        ...
        inline_row.append(InlineKeyboardButton(
            text=f'{self.local_month[calendar.month_name[month]]} {str(year)}',
        ...
        inline_row = []
        inline_row.append(InlineKeyboardButton(
            text="<", callback_data=build_data("PREV-MONTH", year, month, 1)
        ))
        if (year, month) != (datetime.now().year, datetime.now().month):
            ...
            inline_row.append(
                InlineKeyboardButton(text="месяц", callback_data=ignore_callback))

        inline_row.append(InlineKeyboardButton(
            text=">", callback_data=build_data("NEXT-MONTH", year, month, 1)
        ))
        inline_kb.append(inline_row)
        ...
        for week in month_calendar:
            ...
            for day in week:
                ...
                if date.today().month == month and date.today().day == day and date.today().year == year:
                    ...
                    text = str(day)
                inline_row.append(InlineKeyboardButton(
                    text=text, callback_data=build_data("DAY", year, month, day)
                ))
            inline_kb.append(inline_row)
